...
layui.define('jquery', function(exports){
  ...
  exports('code', function(options){
    var elems = [];
    options = options || {};
    options.elem = $(options.elem||'.code');
    ...
    layui.each(elems.reverse(), function(index, item){
      ...
      if(!othis.find('>.code-h3')[0]){
        othis.prepend('<h3 class="code-h3">'+ (othis.attr('lay-title')||options.title||'code') + (options.about ? '<a href="'+ about +'" target="_blank">layui.code</a>' : '') + '</h3>');
      }
      
      var ol = othis.find('>.code-ol');
      othis.addClass('box code-view');
