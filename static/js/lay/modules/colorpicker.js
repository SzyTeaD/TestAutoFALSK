...
layui.define('jquery', function(exports){
  ...
  //字符常量
  ,MOD_NAME = 'colorpicker', SHOW = 'show', THIS = 'this', ELEM = 'colorpicker'
  
  ,ELEM_MAIN = '.colorpicker-main', ICON_PICKER_DOWN = 'icon-down', ICON_PICKER_CLOSE = 'icon-close'
  ,PICKER_TRIG_SPAN = 'colorpicker-trigger-span', PICKER_TRIG_I = 'colorpicker-trigger-i', PICKER_SIDE = 'colorpicker-side', PICKER_SIDE_SLIDER = 'colorpicker-side-slider'
  ,PICKER_BASIS = 'colorpicker-basis', PICKER_ALPHA_BG = 'colorpicker-alpha-bgcolor', PICKER_ALPHA_SLIDER = 'colorpicker-alpha-slider', PICKER_BASIS_CUR = 'colorpicker-basis-cursor', PICKER_INPUT = 'colorpicker-main-input'
  ...
  Class.prototype.render = function(){
    ...
    //颜色选择框对象
    ,elemColorBox = $(['<div class="unselect colorpicker">'
      ,'<span '+ (options.format == 'rgb' && options.alpha
          ? 'class="colorpicker-trigger-bgcolor"'
        : '') +'>'
        ,'<span class="colorpicker-trigger-span" '
          ...
          ,'<i class="icon colorpicker-trigger-i '+ (options.color 
            ? ICON_PICKER_DOWN 
          : ICON_PICKER_CLOSE) +'"></i>'
        ,'</span>'
      ,'</span>'
    ...
    options.size && elemColorBox.addClass('colorpicker-'+ options.size); //初始化颜色选择框尺寸
    
    //插入颜色选择框
    othis.addClass('inline').html(
  ...
  Class.prototype.renderPicker = function(){
    ...
    //颜色选择器对象
    ,elemPicker = that.elemPicker = $(['<div id="colorpicker'+ that.index +'" data-index="'+ that.index +'" class="anim anim-upbit colorpicker-main">'
      //颜色面板
      ,'<div class="colorpicker-main-wrapper">'
        ,'<div class="colorpicker-basis">'
          ,'<div class="colorpicker-basis-white"></div>'
          ,'<div class="colorpicker-basis-black"></div>'
          ,'<div class="colorpicker-basis-cursor"></div>'
        ,'</div>'
        ,'<div class="colorpicker-side">'
          ,'<div class="colorpicker-side-slider"></div>'
        ,'</div>'
      ,'</div>'
      
      //透明度条块
      ,'<div class="colorpicker-main-alpha '+ (options.alpha ? SHOW : '') +'">'
        ,'<div class="colorpicker-alpha-bgcolor">'
          ,'<div class="colorpicker-alpha-slider"></div>'
        ,'</div>'
      ,'</div>'
      ...
      ,function(){
        if(options.predefine){
          var list = ['<div class="colorpicker-main-pre">'];
          layui.each(options.colors, function(i, v){
            list.push(['<div class="colorpicker-pre'+ ((v.match(/[0-9]{1,3}/g) || []).length > 3 
              ? ' colorpicker-pre-isalpha' 
            : '') +'">'
      ...
      //底部表单元素区域
      ,'<div class="colorpicker-main-input">'
        ,'<div class="inline">'
          ,'<input type="text" class="input">'
        ,'</div>'
        ,'<div class="btn-container">'
          ,'<button class="btn btn-primary btn-sm" colorpicker-events="clear">清空</button>'
          ,'<button class="btn btn-sm" colorpicker-events="confirm">确定</button>'
        ,'</div'
      ,'</div>'
    ,'</div>'].join(''))
  ...
  Class.prototype.removePicker = function(index){
    var that = this
    ,options = that.config;
    $('#colorpicker'+ (index || that.index)).remove();
    return that;
  };
  ...
  Class.prototype.side = function(){
    ...
    ,pre = that.elemPicker.find('.colorpicker-pre').children('div')
    ...
    //拖拽元素
    ,elemMove = $(['<div class="auxiliar-moving" id="LAY-colorpicker-moving"></div'].join(''))
