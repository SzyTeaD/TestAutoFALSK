...
layui.define(['layer', 'form'], function(exports){
  ...
  ,MOD_NAME = 'layedit', THIS = 'this', SHOW = 'show', ABLED = 'disabled'
  ...
  Edit.prototype.build = function(id, settings){
    ...
    var that = this
    ,config = that.config
    ,ELEM = 'layedit', textArea = $(typeof(id)=='string'?'#'+id:id)
    ...
    ,editor = $(['<div class="'+ ELEM +'">'
      ,'<div class="unselect layedit-tool">'+ tool +'</div>'
      ,'<div class="layedit-iframe">'
    ...
    if(device.ie && device.ie < 8){
      return textArea.removeClass('hide').addClass(SHOW);
    }

    haveBuild[0] && (haveBuild.remove());

    setIframe.call(that, editor, textArea[0], set)
    textArea.addClass('hide').after(editor);
  ...
  ,toolActive = function(iframeWin, editor, set){
    ...
    ,tools = editor.find('.layedit-tool')
  ...
  ,link = function(options, callback){
    var body = this, index = layer.open({
      ...
      ,title: '超链接'
      ,skin: 'layer-msg'
      ,content: ['<ul class="form" style="margin: 15px;">'
        ,'<li class="form-item">'
          ,'<label class="form-label" style="width: 60px;">URL</label>'
          ,'<div class="input-block" style="margin-left: 90px">'
            ,'<input name="url" lay-verify="url" value="'+ (options.href||'') +'" autofocus="true" autocomplete="off" class="input">'
            ,'</div>'
        ,'</li>'
        ,'<li class="form-item">'
          ,'<label class="form-label" style="width: 60px;">打开方式</label>'
          ,'<div class="input-block" style="margin-left: 90px">'
            ,'<input type="radio" name="target" value="_self" class="input" title="当前窗口"'
            + ((options.target==='_self' || !options.target) ? 'checked' : '') +'>'
            ,'<input type="radio" name="target" value="_blank" class="input" title="新窗口" '
            + (options.target==='_blank' ? 'checked' : '') +'>'
          ,'</div>'
        ,'</li>'
        ,'<li class="form-item" style="text-align: center;">'
          ,'<button type="button" lay-submit lay-filter="layedit-link-yes" class="btn"> 确定 </button>'
          ,'<button style="margin-left: 20px;" type="button" class="btn btn-primary"> 取消 </button>'
        ,'</li>'
      ,'</ul>'].join('')
  ...
  ,face = function(callback){
    ...
    return face.index = layer.tips(function(){
      ...
      return '<ul class="clear">' + content.join('') + '</ul>';
    }(), this, {
      tips: 1
      ,time: 0
      ,skin: 'box util-face'
      ...
      ,success: function(layero, index){
        layero.css({
          marginTop: -4
          ,marginLeft: -10
        }).find('.clear>li').on('click', function(){
          callback && callback({
  ...
  ,code = function(callback){
    var body = this, index = layer.open({
      ...
      ,title: '插入代码'
      ,skin: 'layer-msg'
      ,content: ['<ul class="form form-pane" style="margin: 15px;">'
        ,'<li class="form-item">'
          ,'<label class="form-label">请选择语言</label>'
          ...
          ,'</div>'
        ,'</li>'
        ,'<li class="form-item form-text">'
          ,'<label class="form-label">代码</label>'
          ,'<div class="input-block">'
            ,'<textarea name="code" lay-verify="required" autofocus="true" class="textarea" style="height: 200px;"></textarea>'
          ,'</div>'
        ,'</li>'
        ,'<li class="form-item" style="text-align: center;">'
          ,'<button type="button" lay-submit lay-filter="layedit-code-yes" class="btn"> 确定 </button>'
          ,'<button style="margin-left: 20px;" type="button" class="btn btn-primary"> 取消 </button>'
        ,'</li>'
      ,'</ul>'].join('')
      ,success: function(layero, index){
  ...
  //全部工具
  ,tools = {
    html: '<i class="icon layedit-tool-html" title="HTML源代码" lay-command="html" layedit-event="html"">&#xe64b;</i><span class="layedit-tool-mid"></span>'
    ,strong: '<i class="icon layedit-tool-b" title="加粗" lay-command="Bold" layedit-event="b"">&#xe62b;</i>'
    ,italic: '<i class="icon layedit-tool-i" title="斜体" lay-command="italic" layedit-event="i"">&#xe644;</i>'
    ,underline: '<i class="icon layedit-tool-u" title="下划线" lay-command="underline" layedit-event="u"">&#xe646;</i>'
    ,del: '<i class="icon layedit-tool-d" title="删除线" lay-command="strikeThrough" layedit-event="d"">&#xe64f;</i>'
    
    ,'|': '<span class="layedit-tool-mid"></span>'
    
    ,left: '<i class="icon layedit-tool-left" title="左对齐" lay-command="justifyLeft" layedit-event="left"">&#xe649;</i>'
    ,center: '<i class="icon layedit-tool-center" title="居中对齐" lay-command="justifyCenter" layedit-event="center"">&#xe647;</i>'
    ,right: '<i class="icon layedit-tool-right" title="右对齐" lay-command="justifyRight" layedit-event="right"">&#xe648;</i>'
    ,link: '<i class="icon layedit-tool-link" title="插入链接" layedit-event="link"">&#xe64c;</i>'
    ,unlink: '<i class="icon layedit-tool-unlink disabled" title="清除链接" lay-command="unlink" layedit-event="unlink"">&#xe64d;</i>'
    ,face: '<i class="icon layedit-tool-face" title="表情" layedit-event="face"">&#xe650;</i>'
    ,image: '<i class="icon layedit-tool-image" title="图片" layedit-event="image">&#xe64a;<input type="file" name="file"></i>'
    ,code: '<i class="icon layedit-tool-code" title="插入代码" layedit-event="code">&#xe64e;</i>'
    
    ,help: '<i class="icon layedit-tool-help" title="帮助" layedit-event="help">&#xe607;</i>'
  }
