...
layui.define('jquery',function(exports){
  ...
  //字符常量
  ,MOD_NAME = 'rate',ELEM_VIEW = 'rate', ICON_RATE = 'icon-rate', ICON_RATE_SOLID = 'icon-rate-solid', ICON_RATE_HALF = 'icon-rate-half'
  
  ,ICON_SOLID_HALF = 'icon-rate-solid icon-rate-half',  ICON_SOLID_RATE = 'icon-rate-solid icon-rate',  ICON_HALF_RATE = 'icon-rate icon-rate-half'
  ...
  Class.prototype.render = function(){
    ...
    //组件模板
    var temp = '<ul class="rate" '+ (options.readonly ? 'readonly' : '') +'>';
    for(var i = 1;i <= options.length;i++){
      var item = '<li class="inline"><i class="icon '
      ...
      if(options.half){
        if(parseInt(options.value) !== options.value){
          if(i == Math.ceil(options.value)){
            temp = temp + '<li><i class="icon icon-rate-half" '+ style +'></i></li>';
          }else{
            temp = temp + item 
          } 
        }else{
    ...
    temp += '</ul>' + (options.text ? ('<span class="inline">'+ options.value + '星') : '') + '</span>';
