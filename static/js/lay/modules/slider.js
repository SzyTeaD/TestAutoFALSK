...
layui.define('jquery', function(exports){
  ...
  //字符常量
  ,MOD_NAME = 'slider', DISABLED = 'disabled', ELEM_VIEW = 'slider', SLIDER_BAR = 'slider-bar', SLIDER_WRAP = 'slider-wrap', SLIDER_WRAP_BTN = 'slider-wrap-btn', SLIDER_TIPS = 'slider-tips', SLIDER_INPUT = 'slider-input', SLIDER_INPUT_TXT = 'slider-input-txt', SLIDER_INPUT_BTN = 'slider-input-btn', ELEM_HOVER = 'slider-hover'
  ...
  Class.prototype.render = function(){
    ...
    //滑块
    var temp = '<div class="slider '+ (options.type === 'vertical' ? 'slider-vertical' : '') +'">'+ (options.tips ? '<div class="slider-tips"></div>' : '') + 
    '<div class="slider-bar" style="background:'+ theme +'; '+ (options.type === 'vertical' ? 'height' : 'width') +':'+ scale +';'+ (options.type === 'vertical' ? 'bottom' : 'left') +':'+ (scaleFir || 0) +';"></div><div class="slider-wrap" style="'+ (options.type === 'vertical' ? 'bottom' : 'left') +':'+ (scaleFir || scale) +';">' +
    '<div class="slider-wrap-btn" style="border: 2px solid '+ theme +';"></div></div>'+ (options.range ? '<div class="slider-wrap" style="'+ (options.type === 'vertical' ? 'bottom' : 'left') +':'+ scaleSec +';"><div class="slider-wrap-btn" style="border: 2px solid '+ theme +';"></div></div>' : '') +'</div>';
    ...
    if(options.showstep){
      ...
      for(var i = 1; i < number + 1; i++) {
        ...
        if(step < 100){
          item += '<div class="slider-step" style="'+ (options.type === 'vertical' ? 'bottom' : 'left') +':'+ step +'%"></div>'
        }
      };
    ...
    if(options.input && !options.range){
      var elemInput = $('<div class="slider-input input"><div class="slider-input-txt"><input type="text" class="input"></div><div class="slider-input-btn"><i class="icon icon-up"></i><i class="icon icon-down"></i></div></div>');
  ...
  Class.prototype.slide = function(setValue, value, i){
    ...
    //拖拽元素
    ,elemMove = $(['<div class="auxiliar-moving" id="LAY-slider-moving"></div'].join(''))
