...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function(exports){
  ...
  //字符常量
  ,MOD_NAME = 'table', ELEM = '.table', THIS = 'this', SHOW = 'show', HIDE = 'hide', DISABLED = 'disabled', NONE = 'none'
  
  ,ELEM_VIEW = 'table-view', ELEM_TOOL = '.table-tool', ELEM_BOX = '.table-box', ELEM_INIT = '.table-init', ELEM_HEADER = '.table-header', ELEM_BODY = '.table-body', ELEM_MAIN = '.table-main', ELEM_FIXED = '.table-fixed', ELEM_FIXL = '.table-fixed-l', ELEM_FIXR = '.table-fixed-r', ELEM_TOTAL = '.table-total', ELEM_PAGE = '.table-page', ELEM_SORT = '.table-sort', ELEM_EDIT = 'table-edit', ELEM_HOVER = 'table-hover'
  ...
  ,TPL_HEADER = function(options){
    ...
    options = options || {};
    return ['<table cellspacing="0" cellpadding="0" border="0" class="table" '
      ...
      ,'{{# layui.each(d.data.cols, function(i1, item1){ }}'
        ...
        ,'{{# layui.each(item1, function(i2, item2){ }}'
          ...
          ,'<th data-field="{{ item2.field||i2 }}" data-key="{{d.index}}-{{i1}}-{{i2}}" {{# if( item2.parentKey){ }}data-parentkey="{{ item2.parentKey }}"{{# } }} {{# if(item2.minWidth){ }}data-minwidth="{{item2.minWidth}}"{{# } }} '+ rowCols +' {{# if(item2.unresize || item2.colGroup){ }}data-unresize="true"{{# } }} class="{{# if(item2.hide){ }}hide{{# } }}{{# if(isSort){ }} unselect{{# } }}{{# if(!item2.field){ }} table-col-special{{# } }}">'
            ,'<div class="table-cell laytable-cell-'
              ,'{{# if(item2.colGroup){ }}'
                ,'group'
              ,'{{# } else { }}'
                ,'{{d.index}}-{{i1}}-{{i2}}'
            ...
            ,'" {{#if(item2.align){}}align="{{item2.align}}"{{#}}}>'
              ...
              ,'{{# } else { }}'
                ...
                ,'{{# if(isSort){ }}'
                  ,'<span class="table-sort inline"><i class="edge table-sort-asc" title="升序"></i><i class="edge table-sort-desc" title="降序"></i></span>'
                ,'{{# } }}'
              ,'{{# } }}'
            ,'</div>'
          ,'</th>'
  ...
  //tbody区域模板
  ,TPL_BODY = ['<table cellspacing="0" cellpadding="0" border="0" class="table" '
    ,'{{# if(d.data.skin){ }}lay-skin="{{d.data.skin}}"{{# } }} {{# if(d.data.size){ }}lay-size="{{d.data.size}}"{{# } }} {{# if(d.data.even){ }}lay-even{{# } }}>'
    ,'<tbody></tbody>'
  ,'</table>'].join('')
  
  //主模板
  ,TPL_MAIN = ['<div class="form border-box {{d.VIEW_CLASS}}" lay-filter="LAY-table-{{d.index}}" lay-id="{{ d.data.id }}" style="{{# if(d.data.width){ }}width:{{d.data.width}}px;{{# } }} {{# if(d.data.height){ }}height:{{d.data.height}}px;{{# } }}">'

    ,'{{# if(d.data.toolbar){ }}'
    ,'<div class="table-tool">'
      ,'<div class="table-tool-temp"></div>'
      ,'<div class="table-tool-self"></div>'
    ,'</div>'
    ,'{{# } }}'
    
    ,'<div class="table-box">'
      ,'{{# if(d.data.loading){ }}'
      ,'<div class="table-init" style="background-color: #fff;">'
        ,'<i class="icon icon-loading anim anim-rotate anim-loop"></i>'
      ,'</div>'
      ,'{{# } }}'
      ...
      ,'<div class="table-header">'
        ,TPL_HEADER()
      ,'</div>'
      ,'<div class="table-body table-main">'
        ,TPL_BODY
      ,'</div>'
    ...
    ,'{{# if(d.data.totalRow){ }}'
      ,'<div class="table-total">'
        ...
        ,'{{# if(d.data.skin){ }}lay-skin="{{d.data.skin}}"{{# } }} {{# if(d.data.size){ }}lay-size="{{d.data.size}}"{{# } }} {{# if(d.data.even){ }}lay-even{{# } }}>'
          ,'<tbody><tr><td><div class="table-cell" style="visibility: hidden;">Total</div></td></tr></tbody>'
      , '</table>'
      ,'</div>'
    ,'{{# } }}'
  ...
  Class.prototype.renderToolbar = function(){
    ...
    //添加工具栏左侧模板
    var leftDefaultTemp = [
      '<div class="inline" lay-event="add"><i class="icon icon-add-1"></i></div>'
      ,'<div class="inline" lay-event="update"><i class="icon icon-edit"></i></div>'
      ,'<div class="inline" lay-event="delete"><i class="icon icon-delete"></i></div>'
    ].join('')
    ,elemToolTemp = that.layTool.find('.table-tool-temp');
    ...
    if(typeof options.defaultToolbar === 'object'){
      layui.each(options.defaultToolbar, function(i, item){
        ...
        if(thisItem){
          iconElem.push('<div class="inline" title="'+ thisItem.title +'" lay-event="'+ thisItem.layEvent +'">'
            +'<i class="icon '+ thisItem.icon +'"></i>'
          +'</div>');
        }
      });
    }
    that.layTool.find('.table-tool-self').html(iconElem.join(''));
  }
      ...
      layui.each(data, function(i1, item1){
        ...
        that.eachCols(function(i3, item3){
          ...
          var td = ['<td data-field="'+ field +'" data-key="'+ key +'" '+ function(){ //追加各种属性
            ...
            if(!item3.field) classNames.push('table-col-special'); //插入特殊列样式
            return classNames.join(' ');
          }() +'">'
            ,'<div class="table-cell laytable-cell-'+ function(){ //返回对应的CSS类标识
  ...
  Class.prototype.loading = function(hide){
    ...
    if(options.loading){
      if(hide){
        ...
        that.layInit = $(['<div class="table-init">'
          ,'<i class="icon icon-loading anim anim-rotate anim-loop"></i>'
        ,'</div>'].join(''));
  ...
  Class.prototype.scrollPatch = function(){
    ...
    ,addPatch = function(elem){
      if(scollWidth && scollHeight){
        elem = elem.eq(0);
        if(!elem.find('.table-patch')[0]){
          var patchElem = $('<th class="table-patch"><div class="table-cell"></div></th>'); //补丁元素
  ...
  Class.prototype.events = function(){
    ...
    that.layTool.on('click', '*[lay-event]', function(e){
      ...
      ,openPanel = function(sets){
        var list = $(sets.list)
        ,panel = $('<ul class="table-tool-panel"></ul>');
        ...
        //插入元素
        othis.find('.table-tool-panel')[0] || othis.append(panel);
        that.renderForm();
      ...
      switch(events){
        ...
        case 'LAYTABLE_PRINT': //打印
          ...
          html.find('th.table-patch').remove(); //移除补丁
          html.find('.table-col-special').remove(); //移除特殊列
    ...
    }).find(ELEM_SORT+' .edge ').on('click', function(e){
      var othis = $(this)
      ,index = othis.index()
    ...
    that.layBody.on('click', 'td', function(e){
      ...
      //显示编辑表单
      if(editType){
        var input = $('<input class="input '+ ELEM_EDIT +'">');
    ...
    //单元格展开图标
    var ELEM_GRID = 'table-grid', ELEM_GRID_DOWN = 'table-grid-down', ELEM_GRID_PANEL = 'table-grid-panel'
    ,gridExpand = function(hide){
      ...
      if(hide){
        othis.find('.table-grid-down').remove();
      } else if(elemCell.prop('scrollWidth') > elemCell.outerWidth()){
        if(elemCell.find('.'+ ELEM_GRID_DOWN)[0]) return;
        othis.append('<div class="'+ ELEM_GRID_DOWN +'"><i class="icon icon-down"></i></div>');
      }
    };
    ...
    that.layBody.on('click', '.'+ ELEM_GRID_DOWN, function(e){
      ...
      that.tipsIndex = layer.tips([
        '<div class="table-tips-main" style="margin-top: -'+ (elemCell.height() + 16) +'px;'+ function(){
          if(options.size === 'sm'){
          ...
          ,elemCell.html()
        ,'</div>'
        ,'<i class="icon table-tips-c icon-close"></i>'
      ].join(''), elemCell[0], {
        tips: [3, '']
        ,time: -1
        ,anim: -1
        ...
        ,success: function(layero, index){
          layero.find('.table-tips-c').on('click', function(){
            layer.close(index);
          });
        }
      });
