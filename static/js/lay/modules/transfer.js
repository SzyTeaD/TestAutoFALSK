...
layui.define(['laytpl', 'form'], function(exports){
  ...
  //字符常量
  ,ELEM = 'transfer', HIDE = 'hide', DISABLED = 'btn-disabled', NONE = 'none'
  ,ELEM_BOX = 'transfer-box', ELEM_HEADER = 'transfer-header', ELEM_SEARCH = 'transfer-search', ELEM_ACTIVE = 'transfer-active', ELEM_DATA = 'transfer-data'
  
  //穿梭框模板
  ,TPL_BOX = function(obj){
    obj = obj || {};
    return ['<div class="transfer-box" data-index="'+ obj.index +'">'
      ...
      ,'<div class="transfer-search">'
        ...
        ,'<input type="input" class="input" placeholder="关键词搜索">'
      ,'</div>'
      ,'{{# } }}'
      ,'<ul class="transfer-data"></ul>'
    ,'</div>'].join('');
  }
  
  //主模板
  ,TPL_MAIN = ['<div class="transfer form border-box" lay-filter="LAY-transfer-{{ d.index }}">'
    ,TPL_BOX({
      index: 0
      ,checkAllName: 'layTransferLeftCheckAll'
    })
    ,'<div class="transfer-active">'
      ,'<button type="button" class="btn btn-sm btn-primary btn-disabled" data-index="0">'
        ,'<i class="icon icon-next"></i>'
      ,'</button>'
      ,'<button type="button" class="btn btn-sm btn-primary btn-disabled" data-index="1">'
  ...
  Class.prototype.render = function(){
    ...
    that.layBtn = thisElem.find('.'+ ELEM_ACTIVE + ' .btn');
  ...
  Class.prototype.noneView = function(thisDataElem, text){
    var createNoneElem = $('<p class="none">'+ (text || '') +'</p>');
