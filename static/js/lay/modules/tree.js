...
layui.define('form', function(exports){
  ...
  //字符常量
  ,SHOW = 'show', HIDE = 'hide', NONE = 'none', DISABLED = 'disabled'
  
  ,ELEM_VIEW = 'tree', ELEM_SET = 'tree-set', ICON_CLICK = 'tree-iconClick'
  ,ICON_ADD = 'icon-addition', ICON_SUB = 'icon-subtraction', ELEM_ENTRY = 'tree-entry', ELEM_MAIN = 'tree-main', ELEM_TEXT = 'tree-txt', ELEM_PACK = 'tree-pack', ELEM_SPREAD = 'tree-spread'
  ,ELEM_LINE_SHORT = 'tree-setLineShort', ELEM_SHOW = 'tree-showLine', ELEM_EXTEND = 'tree-lineExtend'
  ...
  Class.prototype.render = function(){
    ...
    var temp = $('<div class="tree'+ (options.showCheckbox ? " form" : "") + (options.showLine ? " tree-line" : "") +'" lay-filter="LAY-tree-'+ that.index +'"></div>');
    that.tree(temp);
    ...
    //插入组件结构
    that.elem = temp;
    that.elemNone = $('<div class="tree-emptyText">'+ options.text.none +'</div>');
    othis.html(that.elem);

    if(that.elem.find('.tree-set').length == 0){
    ...
    that.elem.find('.tree-set').each(function(){
      var othis = $(this);
      //最外层
      if(!othis.parent('.tree-pack')[0]){
        othis.addClass('tree-setHide');
      };

      //没有下一个节点 上一层父级有延伸线
      if(!othis.next()[0] && othis.parents('.tree-pack').eq(1).hasClass('tree-lineExtend')){
        othis.addClass(ELEM_LINE_SHORT);
      };
      
      //没有下一个节点 外层最后一个
      if(!othis.next()[0] && !othis.parents('.tree-set').eq(0).next()[0]){
        othis.addClass(ELEM_LINE_SHORT);
      };
    });
  ...
  Class.prototype.tree = function(elem, children){
    ...
    layui.each(data, function(index, item){
      ...
      ,packDiv = $('<div class="tree-pack" '+ (item.spread ? 'style="display: block;"' : '') +'"></div>')
      ,entryDiv = $(['<div data-id="'+ item.id +'" class="tree-set'+ (item.spread ? " tree-spread" : "") + (item.checked ? " tree-checkedFirst" : "") +'">'
        ,'<div class="tree-entry">'
          ,'<div class="tree-main">'
            ...
            ,function(){
              if(options.showLine){
                if(hasChild){
                  return '<span class="tree-iconClick tree-icon"><i class="icon '+ (item.spread ? "icon-subtraction" : "icon-addition") +'"></i></span>';
                }else{
                  return '<span class="tree-iconClick"><i class="icon icon-file"></i></span>';
                };
              }else{
                return '<span class="tree-iconClick"><i class="tree-iconArrow '+ (hasChild ? "": HIDE) +'"></i></span>';
              };
            }()
      ...
      ,function(){
        ...
        var editIcon = {
          add: '<i class="icon icon-add-1"  data-type="add"></i>'
          ,update: '<i class="icon icon-edit" data-type="update"></i>'
          ,del: '<i class="icon icon-delete" data-type="del"></i>'
        }, arr = ['<div class="btn-group tree-btnGroup">'];
      ...
      if(entryDiv.prev('.'+ELEM_SET)[0]){
        entryDiv.prev().children('.tree-pack').addClass('tree-showLine');
      };
      
      //若无子节点，则父节点加延伸线
      if(!hasChild){
        entryDiv.parent('.tree-pack').addClass('tree-lineExtend');
      };
  ...
  Class.prototype.spread = function(elem, item){
    ...
    touchOpen.on('click', function(e){
      ...
      ,iconClick = touchOpen.children('.icon')[0] ? touchOpen.children('.icon') : touchOpen.find('.tree-icon').children('.icon');
      ...
      if(!packCont[0]){
        ...
        if(elem.hasClass(ELEM_SPREAD)){
          ...
          if(options.accordion){
            ...
            sibls.find('.tree-icon').children('.icon').removeClass(ICON_SUB).addClass(ICON_ADD);
          };
        };
      };
    });
  ...
  Class.prototype.operate = function(elem, item){
    ...
    entry.children('.tree-btnGroup').on('click', '.icon', function(e){
      ...
      if(type == 'add'){
        ...
        if(!packCont[0]){
          //若开启连接线，更改图标样式
          if(options.showLine){
            elemMain.find('.'+ICON_CLICK).addClass('tree-icon');
            elemMain.find('.'+ICON_CLICK).children('.icon').addClass(ICON_ADD).removeClass('icon-file');
          //若未开启连接线，显示箭头
          }else{
            elemMain.find('.tree-iconArrow').removeClass(HIDE);
          };
          //节点添加子节点容器
          elem.append('<div class="tree-pack"></div>');
        };
      ...
      //修改
        ...
        //添加输入框，覆盖在文字上方
        elemMain.append('<input type="text" class="tree-editInput">');
        //获取焦点
        elemMain.children('.tree-editInput').val(text).focus();
        //嵌入文字移除输入框
        ...
        //失去焦点
        elemMain.children('.tree-editInput').blur(function(){
          getVal($(this));
        });
        //回车
        elemMain.children('.tree-editInput').on('keydown', function(e){
          if(e.keyCode === 13){
            e.preventDefault();
            getVal($(this));
          };
      ...
      //删除
        ...
        layer.confirm('确认删除该节点 "<span style="color: #999;">'+ (item.title || '') +'</span>" 吗？', function(index){
          ...
          if(elem.siblings('.'+ELEM_SET).children('.'+ELEM_ENTRY)[0]){
            ...
            //若开启了连接线
            if(options.showLine){
              prevDiv.find('.'+ICON_CLICK).removeClass('tree-icon');
              prevDiv.find('.'+ICON_CLICK).children('.icon').removeClass(ICON_SUB).addClass('icon-file');
              //父节点所在层添加延伸线
            ...
            //父节点隐藏箭头
              prevDiv.find('.tree-iconArrow').addClass(HIDE);
            };
            //移除展开属性
  ...
  Class.prototype.events = function(){
    var that = this
    ,options = that.config
    ,checkWarp = that.elem.find('.tree-checkedFirst');
    
    //初始选中
    that.setChecked(that.checkids);
    
    //搜索
    that.elem.find('.tree-search').on('keyup', function(){
      var input = $(this)
      ,val = input.val()
      ,pack = input.nextAll()
      ,arr = [];
      ...
      pack.find('.'+ELEM_ENTRY).each(function(){
        ...
        if(!parent.hasClass('tree-searchShow')){
          parent.addClass(HIDE);
        };
      });
      if(pack.find('.tree-searchShow').length == 0){
        that.elem.append(that.elemNone);
      };
    ...
    //还原搜索初始状态
    that.elem.find('.tree-search').on('keydown', function(){
      $(this).nextAll().find('.'+ELEM_ENTRY).each(function(){
        var parent = $(this).parent('.'+ELEM_SET);
        parent.removeClass('tree-searchShow '+ HIDE);
      });
      if($('.tree-emptyText')[0]) $('.tree-emptyText').remove();
    });
  };
  ...
  Class.prototype.getChecked = function(){
    ...
    //遍历节点找到选中索引
    that.elem.find('.form-checked').each(function(){
